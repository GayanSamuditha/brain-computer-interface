...
from bci.fourier_transforms.fast_fourier_transform import fast_fourier_transform
...
def butter_bandpass_filter_signal_1d(signal, lowcut, highcut, sample_rate, order, verbose=False):
    ...
    if verbose:
        ...
        plt.ylim([0, 1.2])
        plt.title('Frequency response of filter - lowcut:'+str(lowcut)+', highcut:'+str(highcut))
        plt.show()
...
def butter_bandpass_filter_signal_3d(signal, lowcut, highcut, sample_rate, order, verbose=False):
    r"""
    Digital filter bandpass zero-phase implementation (filtfilt)
    Apply a digital filter forward and backward to a signal
    ...
    Args:
        signal : ndarray, shape (trial,channel,time)
            Input signal by trials in time domain, in 3 dimensional numpy array with shape (trial,channel,time).
        lowcut : int
            Lower bound filter
        highcut : int
            Upper bound filter
    ...
    """
    sos = butter_bandpass(lowcut, highcut, sample_rate, order=order, output='sos')
